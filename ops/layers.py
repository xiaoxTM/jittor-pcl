...
from ops.mops import FurthestPointSampler, KNN
...
class RandPointCNNDecoder(nn.Module):
    """ PointCNN with randomly subsampled representative points. """

    def __init__(self, cin : int, cout : int, clast : int, dims : int, K : int, D : int, P : int) -> None:
        """ See documentation for PointCNN. """
        super(RandPointCNNDecoder, self).__init__()
        self.pointcnn = PointCNN(cin, cout, dims, K, D, P)
        self.P = P
        self.conv_fuse = EndChannels1d(DenseConv1d(cout + clast, cout))
        # if self.P > 0:
...
class PointCNN(nn.Module):
    ...
    def __init__(self, cin : int, cout : int, dims : int, K : int, D : int, P : int) -> None:
        ...
        cmid = cout // 2 if cin == 0 else cout // 4

        if cin == 0:
            depth_multiplier = 4
        else:
            # depth_multiplier = min(int(np.ceil(C_out / C_in)), 4)
            depth_multiplier = int(np.ceil(cout / cin))

        self.knn = KNN(K * D)
        self.dense = EndChannels1d(DenseConv1d(cin, cout // 2)) if cin != 0 else None

        self.xconv = XConv(cout // 2 if cin != 0 else cin, cout, dims, K, P, cmid, depth_multiplier)
    ...
    def execute(self, x):              # (N, P, C_out)
        ...
        fts_p = self.xconv((rep_pts, pts_regional, fts_regional))
        return fts_p
...
class XConv(nn.Module):
    ...
    def __init__(self, cin : int, cout : int, dims : int, K : int,
                 P : int, cmid : int, depth_multiplier : int) -> None:
        ...
        self.dense1 = DenseConv2d(dims, cmid)
        self.dense2 = DenseConv2d(cmid, cmid)
        ...
        self.x_trans_2 = Dense_Conv2d(K*K, K*K, with_bn = False, act = None, groups=1)

        self.end_conv = EndChannels(SepConv(
            in_channels = cmid + cin,
            out_channels = cout,
            kernel_size = (1, K),
